/*--------------------------------------------------------------------------------------
  ...
  1. Select the backend:
       DISPLAY_BACKEND = BACKEND_SPWM  (default), or
  ...
  2. Set DEMO_INDEX:
       ...
       1..18  continuously runs only that scene
  3. Adjust PANEL_TYPE, panel count, row multiplexer, color order and pins.

  BUFFERING
  ---------
  On F4/RP2040, DMD_RGB defaults to two MCU framebuffers. SPWM classes deliberately
  use one MCU staging framebuffer because the panel has its own display SRAM. The panel
  keeps showing its SRAM while a complete new frame is drawn in MCU RAM, then presentFrame()
  uploads it. Both modes therefore present complete frames without drawing directly onto
  the visible image, but SPWM mode does not allocate the library's second MCU framebuffer.
  On STM32F1, DMD_RGB also defaults to one MCU buffer. A typical F103 with 20 KB SRAM
  cannot hold two 4-bit 64x32 frames or even one 4-bit 128x64 frame plus this sketch.

  POWER WARNING
  -------------
  Solid RGB/white tests can draw substantial current. The example starts them dimmed.
  Make sure the panel, wiring and power supply are correctly rated before increasing
  DEFAULT_BRIGHTNESS, SOLID_TEST_BRIGHTNESS or BRIGHTNESS_TEST_MAX.
--------------------------------------------------------------------------------------*/

#define BACKEND_DMD_RGB 1
#define BACKEND_SPWM    2
...
// Library family: BACKEND_SPWM or BACKEND_DMD_RGB.
#ifndef DISPLAY_BACKEND
#define DISPLAY_BACKEND      BACKEND_SPWM
#endif
...
#else
#define PANEL_TYPE           RGB128x64plainS32
#endif
#endif
...
//                                17 pixel text zoom
//                                18 font showcase
#define DEMO_INDEX           0     // 0 = autoplay, 1..18 = one fixed scene
...
#define DEFAULT_BRIGHTNESS    80     // Normal patterns and showcase animations
#define SOLID_TEST_BRIGHTNESS 47     // SPWM shift 3: dimmer than normal full-screen output
#define BRIGHTNESS_TEST_MAX   96     // Brightness test ceiling; use 255 only with a suitable PSU
...
#error "DEMO_INDEX must be from 0 to 18"
#endif
...
#include "DMD_RGB.h"
#elif DISPLAY_BACKEND == BACKEND_SPWM
...
static void presentFrame() {
  // DMD_RGB swaps MCU buffers. SPWM uploads the staged frame into panel SRAM.
  dmd.swapBuffers(false);
}

static uint8_t setOutputBrightness(uint8_t level) {
#if DISPLAY_BACKEND == BACKEND_DMD_RGB
  dmd.setBrightness(level);
  return level;
#else
  // SPWM drivers use binary color-bit dimming; shift 0 is the brightest.
  uint8_t shift;
  if (level >= 192) shift = 0;
  else if (level >= 96) shift = 1;
  else if (level >= 48) shift = 2;
  else if (level >= 24) shift = 3;
  else shift = 4;
  return dmd.shiftColorBrightnessDown(shift);
#endif
}
...
static void renderRGBWSolids(uint32_t elapsed) {
  ...
  setOutputBrightness(SOLID_TEST_BRIGHTNESS);
...
static void renderBrightness(uint32_t elapsed) {
  static const uint8_t levels[] = {
    12, 24, 48, 96, BRIGHTNESS_TEST_MAX, 96, 48, 24
  };
  const int16_t w = dmd.width();
  const int16_t h = dmd.height();
  uint8_t phase = (elapsed / 700UL) % (sizeof(levels) / sizeof(levels[0]));
  uint8_t actualLevel = setOutputBrightness(levels[phase]);
  dmd.fillScreen(BLACK);
...
#if DISPLAY_BACKEND == BACKEND_DMD_RGB
  (void)actualLevel;
  formatByte(levels[phase], valueText);
#else
  valueText[0] = 'S';
...
static uint16_t sceneFrameInterval(uint8_t scene) {
  switch (scene) {
    case TEST_ALIGNMENT: return 180;
    case TEST_GRADIENT: return 180;
    case TEST_RGBW_SOLIDS: return 120;
    case TEST_COLOR_RAMPS: return 180;
    case TEST_PIXEL_HEALTH: return 45;
    case TEST_BRIGHTNESS: return 100;
    case TEST_GHOSTING: return 20;
    case DEMO_PLASMA: return 40;
    case DEMO_NEON_TUNNEL: return 40;
    case DEMO_FIREWORKS: return 32;
    case DEMO_AURORA: return 32;
    case DEMO_MIRROR_PRISM_CALIBRATION: return 45;
    case DEMO_SHOP_PROMO: return 45;
    case DEMO_ALIGNED_MULTILINE_TEXT: return 120;
    case DEMO_HORIZONTAL_TEXT_SCROLL: return (uint16_t)TEXT_SCROLL_STEP_MS;
    case DEMO_VERTICAL_TEXT_SCROLL: return (uint16_t)TEXT_VERTICAL_STEP_MS;
    case DEMO_PIXEL_TEXT_ZOOM:
      return (TEXT_ZOOM_STEP_MS < 70UL) ?
          (uint16_t)TEXT_ZOOM_STEP_MS : 70;
    case DEMO_FONT_SHOWCASE:
      return (TEXT_FONT_HOLD_MS < 100UL) ?
          (uint16_t)TEXT_FONT_HOLD_MS : 100;
    default: return 50;
  }
}
...
static void resetSceneState(uint8_t scene) {
  ...
  setOutputBrightness(DEFAULT_BRIGHTNESS);
  dmd.selectFont(&SmallFont, 1);
...
static void enterScene(uint8_t scene, uint32_t now) {
  activeScene = scene;
  sceneStarted = now;
  lastFrameAt = now - sceneFrameInterval(scene);
  firstFrame = true;
...
void loop() {
  ...
  uint16_t interval = sceneFrameInterval(activeScene);
