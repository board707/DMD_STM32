...
static uint8_t dmdSpwmRegisterTestGlyphRow(char character, uint8_t row)
{
    ...
    if (character == 'R') return letter_r[row];
    if (character == 'E') return letter_e[row];
    if (character == 'G') return letter_g[row];
    return 0;
}
...
static void dmdSpwmDrawRegisterTestScene(DmdType &dmd,
                                         uint16_t catalog_index)
{
#if DMD_SPWM_REGISTER_TEST_PATTERN == DMD_SPWM_REGISTER_TEST_PATTERN_ALIGN
    ...
    DmdType &dmd, const ProfileType &profile, uint8_t word_delay_ms)
{
    ...
    if (!dmd.selectRegisterTestProfile(profile)) return;
