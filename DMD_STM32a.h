...
public:
	...
	DMD(byte mux_cnt, uint8_t* mux_list, byte _pin_nOE, byte _pin_SCLK, byte panelsWide, byte panelsHigh,
		uint8_t n_Rows, bool d_buf = false, byte dmd_pixel_x = 32, byte dmd_pixel_y = 16);
	...
	DMD(byte _pin_A, byte _pin_B, byte _pin_nOE, byte _pin_SCLK, byte panelsWide, byte panelsHigh,
		...
		DMD(2, (uint8_t*)NULL, _pin_nOE, _pin_SCLK, panelsWide, panelsHigh, 4, d_buf, dmd_pixel_x, dmd_pixel_y)
	{
